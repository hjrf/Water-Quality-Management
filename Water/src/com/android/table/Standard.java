...
public class Standard extends Activity implements OnClickListener,AdapterView.OnItemClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {  
	    ...
	    setContentView(R.layout.table_standard);   
	    FindAllView(); 
	    listViewDataBind();
	}  
...
//		checkBoxs.add(checkBox1);  
	...
	public void onClick(View v) {
		...
		case R.id.button2:
			//Log.i(FLAG,"打印的信息！"); 
			sqlite.getReadableDatabase().execSQL("delete from standard where rowid in(select rowid from standard  limit 1 offset '"+ clickItem +"')");
			button2.setEnabled(false);	
		    Intent intent = new Intent();  
//		        intent.putExtra("content", s);  
		    intent.setClass(Standard.this, Standard.class);  
	...
	public void listViewDataBind() {  
		Cursor cursor_standard = sqlite.getReadableDatabase().rawQuery("SELECT * FROM standard",null);
		String[] columnNames = new String[]{cursor_standard.getColumnName(0),cursor_standard.getColumnName(1),
		cursor_standard.getColumnName(2),cursor_standard.getColumnName(3),cursor_standard.getColumnName(4)};
		int columnID [] = new int[] { R.id.id, R.id.par1, R.id.par2, R.id.par3, R.id.par4};
		
//		int filedNum = GetFiledNum("standard");
//		Log.i(TAG,String.valueOf(filedNum));
//		cursor_standard.moveToFirst();
//		do {
//				for(int i = 0;i<filedNum;i++)
//				{
//					Log.i(TAG,cursor_standard.getString(i));
//				}
...
//	 	Log.i(TAG,columnNames[4]);
		...
		ListAdapter adapter = new SetTableStyle(this,  
	            R.layout.table_child_4,cursor_standard,columnNames,columnID);  
	...
	{
		final View viewEditTable = View.inflate(this, R.layout.dialog_standard, null); 
		...
		.setPositiveButton("确认",new DialogInterface.OnClickListener(){
			...
			public void onClick(DialogInterface dialog, int which) {
		    	...
		    	if(way == 0)
		    	{
					...
					{
			    		String sql = "insert into standard (参数名称.一级标准,二级标准,三级标准) values ('"+ et1 +"','"+ et2 +"','"+ et3 +"','"+ et4 +"')";
	...
	//			        intent.putExtra("content", s);  
				...
				{		
		    		if(!"".equals(et1))
		    		{
		    			String sql = "update standard set 参数名称 ='"+ et1 +"' where rowid in(select rowid from standard  limit 1 offset '"+ clickItem +"')";
			    		sqlite.getReadableDatabase().execSQL(sql);
		    		}
		    		if(!"".equals(et2))
		    		{
		    			String sql = "update standard set 一级标准 ='"+ et2 +"' where rowid in(select rowid from standard  limit 1 offset '"+ clickItem +"')";
			    		sqlite.getReadableDatabase().execSQL(sql);
		    		}
		    		if(!"".equals(et3))
		    		{
		    			String sql = "update standard set 二级标准 ='"+ et3 +"' where rowid in(select rowid from standard  limit 1 offset '"+ clickItem +"')";
			    		sqlite.getReadableDatabase().execSQL(sql);
		    		}
		    		if(!"".equals(et4))
		    		{
		    			String sql = "update standard set 三级标准 ='"+ et4 +"' where rowid in(select rowid from standard  limit 1 offset '"+ clickItem +"')";
			    		sqlite.getReadableDatabase().execSQL(sql);
		    		}
		    	       Intent intent = new Intent();  
		    	   	// intent.putExtra("content", s);  
